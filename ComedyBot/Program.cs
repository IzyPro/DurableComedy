...
static async Task RaiseCompletedEvent(HttpClient client, string instanceId, string eventName)
{
    ...
    var response = await ProcessHTTPRequestAsync(client, HttpMethod.Post, baseURL + $"/runtime/webhooks/durabletask/instances/{instanceId}/raiseEvent/{eventName}?taskHub=durablecomedy&connection=Storage&code=5XIu6WghnBJJ-H3Bms5T8ReToVZSBcv491_-teLJ09tBAzFunUAd0Q==", new StringContent(JsonSerializer.Serialize(eventName), Encoding.UTF8, "application/json"));
